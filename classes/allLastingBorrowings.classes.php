...
class AllLastingBorrowings extends DatabaseHandler {

    protected function getAllLastingBorrowings($order, $sort) {
        ...
        if($stmt1->rowCount() == 0) {
            $stmt1 = null;
            echo '<div class="wrapper"><p>Žádné trvalé výpůjčky v databázi</p></div>';
            return;
        }
        ...
        if(isset($_POST["returnButton"])) {
            ...
            $stmt = $this->connect()->prepare('INSERT INTO borrowing_history(user_id_user, copy_id_copy, from_date, extended, user_id_lent_by, lasting, user_id_received_by) VALUES(?, ?, ?, ?, ?, ?, ?);');
        
            if(!$stmt->execute(array($dbBorrowing[0]["user_id_user"], $dbBorrowing[0]["copy_id_copy"], $dbBorrowing[0]["from_date"], $dbBorrowing[0]["extended"], $dbBorrowing[0]["user_id_lent_by"], $dbBorrowing[0]["lasting"], unserialize($_SESSION['user'])->getIdUser()))) {
                $stmt = null;
                echo '<div class="wrapper"><p>stmt failed</p></div>';
                return;
            }
