...
class UserInfo extends DatabaseHandler {
    ...
    protected function getBorrowedBooks($idUser) {
        ...
        'SELECT copy.id_copy, author.name author, book.name book, publisher.name publisher, copy.publication_year, imperfection.name imperfection, borrowing.from_date, borrowing.extended
        FROM author
        ...
        for ($i = 0; $i < $stmt->rowCount(); $i++) {
            echo("<tr><td>" . $dbBorrowing[$i]["author"] . "</td><td>" . $dbBorrowing[$i]["book"] . "</td><td>" . $dbBorrowing[$i]["publisher"] . "</td><td>" . $dbBorrowing[$i]["publication_year"] . "</td><td>" . $dbBorrowing[$i]["from_date"] . "</td><td>" . $dbBorrowing[$i]["extended"] . "</td><td>" . $dbBorrowing[$i]["imperfection"] . "</td></tr>");
        }
        echo("</table></div>");
