...
class AllPublishers extends DatabaseHandler {
    ...
    protected function getAllPublishers() {
        ...
        for ($i = 0; $i < $stmt->rowCount(); $i++) {
            ...
            echo("<tr><td>" . $dbAllPublishers[$i]["name"] . '</td><td><form method="POST"><button type="submit" name="editButton" class="button" value="'.$i.'">Upravit</button></form></td><td><form method="POST"><button type="submit" name="deleteButton" class="button" value="'.$i.'">Odstranit</button></form></td></tr>');
        }
        echo("</table></div>");
