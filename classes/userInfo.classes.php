...
class UserInfo extends DatabaseHandler {
    ...
    protected function getBorrowedBooks($idUser) {
        ...
        for ($i = 0; $i < $stmt1->rowCount(); $i++) {
            ...
            echo("</td><td>" . $dbBorrowing[$i]["book"] . "</td><td>" . $dbBorrowing[$i]["ISBN"] . "</td><td>" . date("d-m-Y", strtotime($dbBorrowing[$i]["from_date"])) . "</td><td>" . date("d-m-Y", strtotime("+1 month", strtotime($dbBorrowing[$i]["from_date"]))) . "</td><td></tr>");
        }
        echo("</table>");
    }
}
